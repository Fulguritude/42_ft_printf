...
static int	check_retvals(int j, int *return_values, char **a_str1, char *str2)
{
	...
	if (j == 2)
		printf(C_YELLOW"/!\\ Note that our string comparison test is not reliable and thus omitted for %%c with argument zero."C_RESET);
//	else if	(j <= 20) //this number is the number of tests before setlocale
//		printf(C_YELLOW"/!\\ Note that our string comparison test is omitted because printf may segfault with multibytes when setlocale has not been called."C_RESET);
	else
	{
