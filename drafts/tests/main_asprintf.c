...
static void	check_retvals(int j, int *return_values, char **a_str1, char *str2)
{
	printf("\n");		
	...
	if (j == 2)
		...
		printf(C_YELLOW"/!\\ Note that our string comparison test is omitted because printf segfaults with multibytes when setlocale has not been called."RESET);
	else
	{
...
*/
	...
	check_retvals(j, return_values, &str1, str2); free(str1); free(str2);
	printf(C_BLUE"\n\nTest %d:"RESET, ++i); printf("\n");
...
printf("\n\n");
	...
	if (fails == 0)
	{
		...
		printf(C_YELLOW"\nTests on return values are mandatory but not sufficient. Check that output values are adequate. Especially on tests where a width flag might return strings of equal lengths.\n"RESET);
	}
	else
